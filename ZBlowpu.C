...
void ZBlowpu::Loop()
{
   ...
   // std::vector<TH1F*> vectptt[1];
    ...
    {
	  /* TH1F *hlt_7   = new TH1F(Form("hlt_7_%d",k),vectrigname[0].c_str(),nx[k],&x[k][0]);
        vectptt[0].push_back (hlt_7);
        hlt_7->Sumw2();*/
	...
	V2daq.push_back(vectptt);vectptt.clear(); //2 boyutlu vektore gectik
    }
  ...
  for (int i=0; i<nentries; i++)  
    {
		...
		int RunNo=t->EvtHdr__mRun;                 
      // if( RunNo != 307075 && RunNo!= 307046 &&  RunNo !=306896  && RunNo != 306897){
        ...
        for(int j=0; j<t->PFJetsCHS__; j++)
        {
            ...
            vector<double>vecttrigcuts={15,49}; //10Mart
            vector<double>vecttrigcuts1={15,49};
            vector<double>vecttrigcuts2={15,56};
                    ...
                    if ( fabs(pf_jteta)>=etabins[k] && fabs(pf_jteta)<etabins[k+1]){
                        ...
                        for(int trnameindex=0; trnameindex<1; trnameindex++)
                        {
							for(int trdecindex=0; trdecindex<t->TriggerDecision_.size(); trdecindex++)
                            {
                                ...
                                {
                         //vectptt[trnameindex][k]->Fill(pf_pt,1./458.494); 
                          //vectptt[trnameindex][k]->Fill(pf_pt,1./4753.470);
						 // vectptt[trnameindex][k]->Fill(pf_pt,1./215.044658174);  
						  //vectptt[trnameindex][k]->Fill(pf_pt);                      
                        //vectptt[k]->Fill(name,t->PFJetsCHS__chf_[j]);
